import { z } from "zod";
import { stripCategoryNumber, stripGroupNumber } from "@/lib/taxonomy-label";
...
// parentId.
// Numbers are derived from position now (lib/taxonomy-label.ts), so one typed
// into the front of a name is dropped rather than stored — otherwise the list
// would read "1.2 1.2 งานผนัง" and the two could disagree.
const groupName = z
  .string()
  .transform(stripGroupNumber)
  .refine((name) => name.length > 0, "กรุณาระบุชื่อ");

const categoryName = z
  .string()
  .transform(stripCategoryNumber)
  .refine((name) => name.length > 0, "กรุณาระบุชื่อ");
